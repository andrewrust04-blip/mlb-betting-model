...
#
# This version matches the updated model feature set:
#   1. rolling_K_rate_last_5
#   2. blended_K_rate
#   3. rolling_IP_last_5
#   4. rolling_batters_faced_last_5
#   5. rolling_pitch_count_last_5
#   6. days_rest
#   7. smoothed_opponent_k_pct
#   8. home_away
#
# Important changes:
#   - Removed the old 0.975 mean reduction.
#   - Uses one minimum edge threshold of 10% for both overs and unders.
#   - Uses the same weighted opponent K% and blended pitcher K-rate logic as the
#     updated dataset/training pipeline.
...
from config_github import (
    ...
    SIM_CLIP_MAX,
    PROB_THRESHOLDS,
    HALF_LINE_THRESHOLDS,
    MODEL_PATH,
    SCALER_PATH,
    SPORTSBOOK_LINES_PATH,
    LIVE_PREDICTIONS_PATH,
    FILTERED_BETS_PATH,
    BET_LOG_PATH,
    ALLOW_MULTIPLE_BETS_PER_PITCHER,
    LEAGUE_K_PCT,
...
# One threshold for both overs and unders.
# This replaces the old over=12% and under=8% side-specific thresholds.
MIN_BET_EDGE_THRESHOLD = 0.10
...
def safe_divide(numerator, denominator):
    """Safely divide two numeric Series without pandas nullable NA boolean errors."""
    ...
    valid_mask = den.gt(0).fillna(False)
...
def normalize_name(name):
    """Normalize names so sportsbook names and MLB schedule names match better."""
    if pd.isna(name):
        return ""
...
def get_bet_size(edge):
    """Simple unit sizing based on edge, with 10% as the minimum bet threshold."""
    if pd.isna(edge) or edge < MIN_BET_EDGE_THRESHOLD:
        return 0.0
    if edge < 0.125:
        return 0.5
    if edge < 0.150:
...
def kelly_fraction(prob, odds):
    """Calculate raw Kelly fraction from probability and American odds."""
...
def get_kelly_units(prob, odds, bankroll_fraction=0.25, max_units=2.0):
    """Convert Kelly fraction into capped unit recommendation."""
...
def simulate_nb_draws(mu_array, alpha, n_sims, clip_max, rng):
    """
    Draw Negative Binomial samples for each predicted mean.
...
def get_empty_filtered_bets_df():
    """Return an empty filtered-bets table with the expected output columns."""
...
def get_probable_pitchers_for_date(target_date):
    """Pull probable starters from the MLB Stats API schedule endpoint."""
    ...
    for date_block in data.get("dates", []):
        for game in date_block.get("games", []):
            ...
            home_abbr = TEAM_ABBR_MAP.get(home_name)
            away_abbr = TEAM_ABBR_MAP.get(away_name)

            home_probable = home.get("probablePitcher", {})
            away_probable = away.get("probablePitcher", {})

            # IMPORTANT:
            # build_dataset.py trains home_away as 1 for home pitchers and 0 for away pitchers.
            if home_probable:
                rows.append({
                    "date": pd.to_datetime(target_date),
                    "game_pk": game_pk,
                    "pitcher_id": home_probable.get("id"),
                    "pitcher_name": home_probable.get("fullName"),
                    "team": home_abbr,
                    "opponent": away_abbr,
                    "home_away": 1,
                })
            ...
            if away_probable:
                rows.append({
                    "date": pd.to_datetime(target_date),
                    "game_pk": game_pk,
                    "pitcher_id": away_probable.get("id"),
                    "pitcher_name": away_probable.get("fullName"),
                    "team": away_abbr,
                    "opponent": home_abbr,
                    "home_away": 0,
                })
...
def build_live_features(live_df, hist_df, target_date):
    """
    Build live feature rows using the same definitions as the training dataset.

    Uses only historical rows before target_date, so there is no same-day leakage.
    """
    ...
    numeric_cols = [
        "strikeouts", "batters_faced", "innings_pitched", "pitch_count", "game_K_rate",
    ]
    for col in numeric_cols:
        if col in hist_prior.columns:
            hist_prior[col] = pd.to_numeric(hist_prior[col], errors="coerce")
    ...
    for _, row in live_df.iterrows():
        ...
        if pd.isna(pitcher_id):
            skipped_rows.append({
                ...
                "reason": "missing pitcher_id from schedule",
            })
            continue
        ...
        if pd.isna(team) or pd.isna(opponent):
            skipped_rows.append({
                "pitcher_name": pitcher_name,
                "reason": "missing team/opponent abbreviation mapping",
            })
            continue
        ...
        # The model was trained after dropping rows without 5 prior starts.
        # So live predictions also require at least 5 prior starts.
        if len(pitcher_hist) < 5:
            skipped_rows.append({
                "pitcher_name": pitcher_name,
                "reason": "fewer than 5 prior starts in dataset",
            })
            continue

        last_5 = pitcher_hist.tail(5)

        rolling_k_last_5 = last_5["strikeouts"].sum()
        rolling_bf_last_5 = last_5["batters_faced"].sum()

        rolling_K_rate_last_5 = (
            rolling_k_last_5 / rolling_bf_last_5
            if rolling_bf_last_5 > 0
            else np.nan
        )

        rolling_IP_last_5 = last_5["innings_pitched"].mean()
        rolling_batters_faced_last_5 = last_5["batters_faced"].mean()
        rolling_pitch_count_last_5 = last_5["pitch_count"].mean()

        last_start_date = pitcher_hist["date"].max()
        days_rest = (target_ts - last_start_date).days
        days_rest = min(max(days_rest, 0), DAYS_REST_CAP)
        ...
        season_bf = season_hist["batters_faced"].sum()
        season_K_rate = season_k / season_bf if season_bf > 0 else np.nan

        prior_k = prior_season_hist["strikeouts"].sum()
        prior_bf = prior_season_hist["batters_faced"].sum()
        prior_season_K_rate = prior_k / prior_bf if prior_bf > 0 else np.nan

        pitcher_current_weight = (
            season_bf / (season_bf + PITCHER_K_RATE_STABILIZER_BF)
            if season_bf > 0
            else 0.0
        )

        pitcher_prior_component = (
            prior_season_K_rate
            if pd.notna(prior_season_K_rate)
            else LEAGUE_K_PCT
        )
        pitcher_current_component = (
            season_K_rate
            if pd.notna(season_K_rate)
            else pitcher_prior_component
        )

        blended_K_rate = (
            pitcher_current_weight * pitcher_current_component
            + (1.0 - pitcher_current_weight) * pitcher_prior_component
        )
        ...
        opp_current = hist_prior[
            (hist_prior["season"] == target_season)
            & (hist_prior["opponent"] == opponent)
        ].copy()

        opp_prior = hist_prior[
            (hist_prior["season"] == prior_season)
            & (hist_prior["opponent"] == opponent)
        ].copy()

        opp_current_k = opp_current["strikeouts"].sum()
        opp_current_bf = opp_current["batters_faced"].sum()
        current_season_opponent_k_pct = (
            opp_current_k / opp_current_bf
            if opp_current_bf > 0
            else np.nan
        )

        opp_prior_k = opp_prior["strikeouts"].sum()
        opp_prior_bf = opp_prior["batters_faced"].sum()
        prior_opponent_k_pct = (
            opp_prior_k / opp_prior_bf
            if opp_prior_bf > 0
            else np.nan
        )

        opponent_current_weight = (
            opp_current_bf / (opp_current_bf + OPPONENT_K_STABILIZER_BF)
            if opp_current_bf > 0
            else 0.0
        )

        opponent_prior_component = (
            prior_opponent_k_pct
            if pd.notna(prior_opponent_k_pct)
            else LEAGUE_K_PCT
        )
        opponent_current_component = (
            current_season_opponent_k_pct
            if pd.notna(current_season_opponent_k_pct)
            else opponent_prior_component
        )

        smoothed_opponent_k_pct = (
            opponent_current_weight * opponent_current_component
            + (1.0 - opponent_current_weight) * opponent_prior_component
        )
        ...
        feature_rows.append({
            ...
            # Model features
            "rolling_K_rate_last_5": rolling_K_rate_last_5,
            "blended_K_rate": blended_K_rate,
            "rolling_IP_last_5": rolling_IP_last_5,
            "rolling_batters_faced_last_5": rolling_batters_faced_last_5,
            "rolling_pitch_count_last_5": rolling_pitch_count_last_5,
            "days_rest": days_rest,
            "smoothed_opponent_k_pct": smoothed_opponent_k_pct,

            # Diagnostics
            "rolling_K_last_5": last_5["strikeouts"].mean(),
            "season_K_rate": season_K_rate,
            "prior_season_K_rate": prior_season_K_rate,
            "pitcher_current_weight": pitcher_current_weight,
            "current_season_opponent_k_pct": current_season_opponent_k_pct,
            "prior_opponent_k_pct": prior_opponent_k_pct,
            "opponent_current_weight": opponent_current_weight,
    ...
    if not features_df.empty:
        ...
        bad_feature_rows = features_df[features_df[FEATURE_COLS].isna().any(axis=1)].copy()
        if not bad_feature_rows.empty:
            for _, bad_row in bad_feature_rows.iterrows():
                missing_features = [
                    col for col in FEATURE_COLS
                    if pd.isna(bad_row[col])
                ]
                skipped_rows.append({
                    "pitcher_name": bad_row.get("pitcher_name", "unknown"),
                    "reason": f"missing live features: {missing_features}",
                })

            features_df = features_df.dropna(subset=FEATURE_COLS).copy()
            skipped_df = pd.DataFrame(skipped_rows)
...
def load_and_parse_sportsbook_lines(target_date):
    """Load today's sportsbook strikeout lines and reshape to one row per pitcher-line."""
...
missing_dataset_features = [col for col in FEATURE_COLS if col not in hist_df.columns]
if missing_dataset_features:
    raise ValueError(
        "Historical dataset is missing model feature columns.\n"
        f"Missing: {missing_dataset_features}\n\n"
        "Rebuild pitcher_game_features.parquet with the updated build_dataset.py, "
        "then commit/push the updated parquet file before running this script."
    )
...
X_live_scaled_array = scaler.transform(X_live)
...
mu_live = np.asarray(nb_model.predict(X_live_nb), dtype=float)
mu_live = np.clip(mu_live, 1e-9, None)

# No manual mean reduction here.
# The old line `mu_live = mu_live * 0.975` has been removed.
...
sim_median = np.median(draws, axis=1)
sim_std = draws.std(axis=1)
...
# Keep model features in the live output for diagnostics.
for col in FEATURE_COLS:
    if col not in results_cols:
        results_cols.append(col)

results = features_df[results_cols].copy()
...
if not lines_wide.empty:
    for line in HALF_LINE_THRESHOLDS:
        ...
        betting_df[f"edge_over_{label}"] = np.round(
            betting_df[f"p_over_{label}"] - betting_df[f"implied_over_{label}"],
            4,
        )
        betting_df[f"edge_under_{label}"] = np.round(
            betting_df[f"p_under_{label}"] - betting_df[f"implied_under_{label}"],
            4,
        )
    ...
    for _, row in betting_df.iterrows():
        for line in HALF_LINE_THRESHOLDS:
            ...
            for side in ("over", "under"):
                ...
                if pd.notna(edge_val) and pd.notna(odds_val) and edge_val >= MIN_BET_EDGE_THRESHOLD:
                    bet_rows.append({
                        "date": row["date"],
                        "season": row["season"],
                        "game_pk": row["game_pk"],
    ...
    if not filtered_bets.empty:
        if not ALLOW_MULTIPLE_BETS_PER_PITCHER:
            filtered_bets = (
                ...
                .groupby(["date", "pitcher_id"], as_index=False)
                .head(1)
        ...
        filtered_bets["kelly_units"] = filtered_bets.apply(
            lambda r: get_kelly_units(r["model_prob"], r["odds"]),
            axis=1,
        )
        ...
        print(f"\nNo bets met the edge threshold of {MIN_BET_EDGE_THRESHOLD:.1%}.")
...
# =============================================================================
# STEP 7: SAVE CLEAN LIVE BETTING VIEW
# =============================================================================
...
print("\nlive_predictions_github.py complete.")